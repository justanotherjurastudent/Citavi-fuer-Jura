...
namespace SwissAcademic.Citavi.Citations
{
    public class ComponentPartFilter : IComponentPartFilter
    {
        public IEnumerable<ITextUnit> GetTextUnits(ComponentPart componentPart, Template template, Citation citation, out bool handled)
        {
            ...
            PageRangeFieldElement pageRangeFieldElement;
            QuotationPageRangeFieldElement quotationPageRangeFieldElement;
            if (!TryValidateComponentPartStructure(componentPart, out pageRangeFieldElement, out quotationPageRangeFieldElement))
            {
                return null;
            }

            var articlePageRange = citation.Reference.PageRange;
            if (!citation.Reference.HasCoreField(ReferenceTypeCoreFieldId.PageRange) ||
                articlePageRange == null ||
                string.IsNullOrWhiteSpace(articlePageRange.PrettyString))
            {
            ...
            var quotationPageRange = placeholderCitation.Entry.PageRange;
            if (quotationPageRange == null || string.IsNullOrWhiteSpace(quotationPageRange.PrettyString))
            {
                ApplyStartPageOnlyFormatting(pageRangeFieldElement);
                RemoveAllButElement(componentPart, pageRangeFieldElement);
                return null;
            }

            var quotationText = quotationPageRange.PrettyString.Trim();
            var articleStartPageText = articlePageRange.StartPage.ToString();

            if (!string.IsNullOrEmpty(articleStartPageText) && quotationText == articleStartPageText)
            {
                TransferSettings(pageRangeFieldElement, quotationPageRangeFieldElement);
                RemoveAllButElement(componentPart, quotationPageRangeFieldElement);
                RemoveParenthesesFromPrefixSuffix(quotationPageRangeFieldElement);
            ...
            string language = GetCitationLanguage(componentPart, citation);
        ...
        private static string GetCitationLanguage(ComponentPart componentPart, Citation citation)
        {
        ...
        private static void ApplyStartPageOnlyFormatting(PageRangeFieldElement element)
        {
            element.PageOneHasSpecialFormat = false;
            element.PageTwoHasSpecialFormat = false;
            element.PageMultiNumberingStyle = NumberingStyle.StartPageOnly;
            element.PageMultiSuffix.Text = string.Empty;

            element.ColumnOneHasSpecialFormat = false;
            element.ColumnTwoHasSpecialFormat = false;
            element.ColumnMultiNumberingStyle = NumberingStyle.StartPageOnly;
            element.ColumnMultiSuffix.Text = string.Empty;

            element.ParagraphOneHasSpecialFormat = false;
            element.ParagraphTwoHasSpecialFormat = false;
            element.ParagraphMultiNumberingStyle = NumberingStyle.StartPageOnly;
            element.ParagraphMultiSuffix.Text = string.Empty;

            element.MarginOneHasSpecialFormat = false;
            element.MarginTwoHasSpecialFormat = false;
            element.MarginMultiNumberingStyle = NumberingStyle.StartPageOnly;
            element.MarginMultiSuffix.Text = string.Empty;

            element.OtherOneHasSpecialFormat = false;
            element.OtherTwoHasSpecialFormat = false;
            element.OtherMultiNumberingStyle = NumberingStyle.StartPageOnly;
            element.OtherMultiSuffix.Text = string.Empty;
        }
